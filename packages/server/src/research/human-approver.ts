...
try{request=JSON.parse(params.get('request'));replyOrigin=params.get('replyOrigin');if(!request||!/^https?:/.test(replyOrigin))throw Error();document.getElementById('request').textContent=JSON.stringify(request,null,2);const scope=request.body.scope||{},limits=scope.preparationLimits||scope.executionLimits||scope.formalResources||{},display=scope.display||{},taskNames={'synthetic-summary-v1':'摘要检查','synthetic-evaluation-v1':'合成评分评估','synthetic-training-evaluation-v1':'合成特征评估','synthetic-retinal-image-v1':'合成眼底图像实验','supervised-phantom-v2':'真实训练：合成图像双模型比较'},rows=[['研究项目',scope.costResearchTitle||display.title||'当前研究项目'],['任务',taskNames[display.task]||'当前冻结任务'],['修订',display.revision||'按签署方案'],['审批类型',({execution:'实验执行',formal_code_review:'候选代码独立审阅（不执行代码）',formal_execution:'正式隔离实验（精确冻结计划）',cli_preparation:'远端 CLI 代码准备（候选不自动执行）',model_review:'独立复核',protocol:'研究方案',release:'结论发布',cost_settlement:'费用依据结算（不授权新增支出）',controller:'有界主控推进（实验仍需单独批准）'})[scope.kind]||request.action],[scope.kind==='cli_preparation'?'资金预留（实际费用未知，非收费硬限）':'预算',(scope.maxCost??'未提供')+' '+(scope.currency||'')],...(scope.kind==='cost_settlement'||scope.kind==='controller'?[]:[['最长运行',limits.maxRuntimeMs?limits.maxRuntimeMs/60000+' 分钟':'按已批准方案'],[scope.kind==='cli_preparation'?'计划申请资源（非系统硬限）':'计算资源',limits.cpu?limits.cpu+' CPU / '+limits.memoryMb+' MB':'按已批准方案'],['关联产物',(scope.artifactVersionIds||[]).length+' 项']]),...(scope.kind==='controller'?[['模型请求上限',scope.controllerLimits?.maxModelRequests],['推进操作上限',scope.controllerLimits?.maxAdvances],['截止时间',new Date(scope.controllerLimits?.deadlineAt).toLocaleString()],['停止阶段',scope.controllerLimits?.stopAfter==='candidate'?'获得候选代码':'完成独立复核'],['单请求输出上限',scope.controllerLimits?.maxOutputTokens],['单请求输入字符上限',scope.controllerLimits?.maxInputCharacters]]:[]),...(scope.kind==='cost_settlement'?[['结算对象',scope.costSubjectLabel||'待核对'],['结算金额',scope.costAmount+' '+scope.currency],['结算依据',scope.costDescription||'未提供'],['依据性质','人工核账，不等同于服务商自动测量']]:[])];document.getElementById('summary').textContent='';for(const [label,value] of rows){const p=document.createElement('p');p.textContent=label+'：'+value;document.getElementById('summary').appendChild(p);}}catch{}
