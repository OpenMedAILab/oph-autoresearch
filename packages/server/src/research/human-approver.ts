...
try{request=JSON.parse(params.get('request'));replyOrigin=params.get('replyOrigin');if(!request||!/^https?:/.test(replyOrigin))throw Error();document.getElementById('request').textContent=JSON.stringify(request,null,2);}catch{}
