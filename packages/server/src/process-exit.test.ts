...
describe('上一份 sidecar 的退出现场', () => {
  test('退出码、信号、时间和 stderr 原样解析，不用真假值吞掉退出码 0', () => {
    expect(
      processExitObservationFromEnv({
        OPH_AUTORESEARCH_PREVIOUS_EXIT_KIND: 'terminated',
        OPH_AUTORESEARCH_PREVIOUS_EXIT_AT_MS: '1234',
        OPH_AUTORESEARCH_PREVIOUS_EXIT_CODE: '0',
        OPH_AUTORESEARCH_PREVIOUS_EXIT_SIGNAL: '9',
        OPH_AUTORESEARCH_PREVIOUS_STDERR_TAIL: 'panic at worker.ts:8',
      }),
    ).toEqual({
  ...
  test('缺少可信的种类或时间就忽略，不能把普通 CLI 启动编成崩溃恢复', () => {
    expect(
      processExitObservationFromEnv({ OPH_AUTORESEARCH_PREVIOUS_EXIT_CODE: '1' }),
    ).toBeUndefined()
    expect(
      processExitObservationFromEnv({
        OPH_AUTORESEARCH_PREVIOUS_EXIT_KIND: 'terminated',
        OPH_AUTORESEARCH_PREVIOUS_EXIT_AT_MS: 'not-a-number',
      }),
    ).toBeUndefined()
  })
  ...
  test('stderr 在落库前按已知值与 key 形状脱敏', () => {
    const observation = processExitObservationFromEnv({
      OPH_AUTORESEARCH_PREVIOUS_EXIT_KIND: 'terminated',
      OPH_AUTORESEARCH_PREVIOUS_EXIT_AT_MS: '1234',
      OPH_AUTORESEARCH_PREVIOUS_STDERR_TAIL: 'Authorization: sk-test-secret-1234567890',
    })!
