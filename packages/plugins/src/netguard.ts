/**
 * 出网闸：把 `host.net.fetch` 变成插件**唯一**无需绕行即可使用的出网通道。
 *
 * **为什么需要它。** Node 的权限模型（`--permission`）覆盖文件系统、子进程、worker、原生插件，
 * **唯独不管网络**。所以沙箱把「插件能读到什么」从整个主目录缩到了一个工作区，
 * 却没有堵住外发通道——它照样可以把读到的内容发出去。
 * 那道墙缺的就是这一面。
 *
 * **说清楚这道闸是什么、不是什么。** 它是**进程内的拆除**，不是内核边界。准确的说法是
 ...
 *    后者要起 worker，而 `--allow-worker` 这个旗子一律不给——给了它，
 ...
 *    本文件的钩子不会被调到。而 ESM 命名空间的属性**不能重定义**
 ...
 * | 插件自己 `registerHooks` 短路掉本文件的钩子 | 把 `node:module` 一并列入黑名单 |
 ...
 * 挡掉的模块。
 *
 * **只管网络，不重复权限模型已经管的事。** `child_process` 和 `worker_threads` **刻意不在这张表里*
 * *。它们由 `--permission` 管：没声明 `process:exec` 就拿不到 `--allow-child-process`，而
 * `--allow-worker` 一律不给。把它们也写进这里的话，一个**被明确授予** `process:exec` 的插件会拿到
 * `--allow-child-process` 旗子、却在模块层被挡掉——两套机制对同一件事给出相反的答案，那种自相矛
 * 盾比缺一层防护更难查。
 *
