 ...
 *
 * **只动 `roles`，绝不碰 `rules`。** `rules` 是用户对这台机器定的纪律：`shared`（追加给所有角色的公
 * 共约束）与 `maxConcurrent`（一张图里同时最多几个节点在跑，也就是同时烧几份钱）。让模型整份改写这个
 * 文件，它就能一并改掉这两条。所以这里读出原文、只替换 `roles` 里的一条、其余键原样写回。
 */
