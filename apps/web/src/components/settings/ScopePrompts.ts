...
export function newMemoryPrompt(scope: Scope): string {
  return (
    ...
    '先说明记忆在 oph-autoresearch 里怎么工作、分哪几层、写在哪个目录；然后问我要记什么。' +
...
export function newSkillPrompt(scope: Scope): string {
  return (
    ...
    '先说明技能在 oph-autoresearch 里怎么被索引、什么时候会被加载，目录和 SKILL.md 长什么样；' +
    '然后问我这个技能要干什么、分几步。' +
...
export function newMcpPrompt(scope: Scope): string {
  return (
    ...
    '先说明 MCP 服务在 oph-autoresearch 里怎么配置、连接，配置写在哪个文件；' +
