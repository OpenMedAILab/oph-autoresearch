...
```mermaid
flowchart LR
    A["研究者提出临床问题"] --> B["研究协调 Agent<br/>可行性分析与研究设计"]
    B --> C{"人工检查点<br/>冻结研究方案"}
    C -->|退回修改| B
    C -->|批准| D["数据审计 Agent<br/>只读检查 SSH 数据"]
    D --> E["脱敏数据清单<br/>队列、标签与泄漏风险"]
    E --> F["实验执行 Agent<br/>在 SSH/GPU 节点训练与评估"]
    F --> G["运行回执<br/>配置、代码版本、指标与日志"]
    G --> H["独立复核 Agent<br/>统计检验与负结果分析"]
    H --> I{"结论是否有充分证据？"}
    I -->|否| J["修订假设、数据或实验方案"]
    J --> F
    I -->|是| K["图表、方法描述与论文草稿"]
    ...
    N["本地脱敏数据"] -.-> B
    O["远程原始眼科影像<br/>始终留在 SSH 服务器"] -.-> D
    P["可配置模型 / CLI<br/>Claude Code · Codex · 其他模型"] -.-> B
    P -.-> D
    P -.-> F
    P -.-> H
```
