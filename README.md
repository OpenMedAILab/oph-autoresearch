...
- 直接从聊天主题启动“文献与刊会调研 → 方案卡确认 → 子代理实验准备 → 已授权实验 → 结果复核 → 写作与审稿”。前期调研不要求配置正式实验执行器。
...
- 默认提供文献研究、刊会分析、数据审计、方案统计、实验准备与执行、证据写作，以及临床、方法学、稿件等独立审阅角色；角色绑定职责与必读 Skill。
...
主控负责使用现有 workflow 实际派发任务，自动核验内部交接；人类确认聚焦研究方案版本。实验执行、预算和产物回执仍沿用已有实现。没有真实结果时不能把提纲或占位文字保存成完整结果稿。旧六阶段详情保留在“高级详情”，不要求用户在日常聊天中操作底层状态机。
