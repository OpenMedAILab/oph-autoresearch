# oph-autoresearch — 项目记忆索引

oph-autoresearch 自己的记忆放这里，一条一个文件，下面一行一条指向它：`- [标题](文件.md) — 一句话`。

**分层规则**（见 `CLAUDE.md` D3）：只跟 oph-autoresearch 这份代码有关的写这里；
...
- [排查先查本地数据库](diagnose-from-the-local-db.md) — `~/.oph-autoresearch/oph-autoresearch.sqlite3` 有每次工具调用的完整 args 与 outcome；`permission_audit` 空表 = 一次权限裁决都没发生过
...
- [PTY 还在不在看进程树](pty-alive-check-by-process-tree.md) — `oph-autoresearch.exe` 下有 `conhost.exe --headless` + shell 才算活着；只剩 webview 就是界面在骗人
