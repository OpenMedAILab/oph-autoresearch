...
description: 判断终端页背后的 PTY 还在不在，看 oph-autoresearch.exe 有没有 conhost 子进程
metadata:
  type: project
---
...
```powershell
Get-CimInstance Win32_Process -Filter "ParentProcessId=<oph-autoresearch.exe 的 pid>"
```

一条活着的终端会话在 Windows 上是 `conhost.exe --headless --inheritcursor --width N --height N`
加一个 `powershell.exe`，两者都是 `oph-autoresearch.exe` 的直接子进程。只剩 `msedgewebview2.exe` = Rust 侧的会话表是空的，
