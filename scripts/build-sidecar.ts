...
function windowsMetadata(version: string): string[] {
  ...
  return [
    '--windows-title=oph-autoresearch',
    '--windows-publisher=OpenMedAILab',
    '--windows-description=oph-autoresearch agent runtime',
    `--windows-version=${version.split('-')[0]}.0`,
    '--windows-copyright=OpenMedAILab; MIT AND Apache-2.0',
    `--windows-icon=${ICON}`,
  ]
}
