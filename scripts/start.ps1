<#
.SYNOPSIS
  oph-autoresearch 一键启动。

.DESCRIPTION
  两种模式：

    desktop（默认）  Tauri 原生窗口。外壳自己用 `--port 0` 拉一个 oph sidecar，
                     不占固定端口；前端走 vite dev server，改代码热更新。
    web              浏览器 / 手机。oph serve 固定在 7717，vite 在 5180 代理过去，
                     脚本把带令牌的地址打出来并自动开浏览器。
...
# --- 端口清场 -----------------------------------------------------------------
# 只清开发进程（node / bun / vite / oph / oph-autoresearch）。端口被别的进程占着就停下来
...
function Clear-DevPort([int]$Port) {
  ...
  foreach ($c in $conns) {
    ...
    if ($proc -and $proc.ProcessName -notin @('node', 'bun', 'vite', 'oph', 'oph-autoresearch')) {
      throw "端口 $Port 被 $name (pid $($c.OwningProcess)) 占用，不像是 oph-autoresearch 的开发进程，脚本不动它。请自行确认后处理。"
    }
...
# 但它**占着 `.tmp\cargo-target\debug\oph.exe` 的文件句柄**——tauri-build 要把新的 sidecar
# 复制过去，复制失败后整个 dev 构建以 exit 101 退出，报出来的只有一句
# 「拒绝访问」，完全看不出和上一个还在跑的窗口有关。实测踩到过。
#
# 只清本仓 target 目录下的那两个可执行文件，路径不匹配的同名进程一律不动
# ——机器上可能装着正式版 oph-autoresearch。
function Clear-StaleShell {
  $root = (Resolve-Path $PSScriptRoot\..).Path
  foreach ($p in @(Get-Process oph-autoresearch, oph -ErrorAction SilentlyContinue)) {
...
$configFile = if ($env:OPH_AUTORESEARCH_HOME) {
  Join-Path $env:OPH_AUTORESEARCH_HOME 'config.json'
} else {
  Join-Path $env:USERPROFILE '.oph-autoresearch\config.json'
}
...
if ($Mode -eq 'desktop') {
  ...
  #
  # 直接跑 tauri dev 的话，外壳会 spawn `externalBin` 那个预编译的 bin/oph——
  ...
  #
  # 工作区钉到仓库根这件事由 dev.ts 自己做（它 spawn 外壳时设 OPH_AUTORESEARCH_WORKSPACE）,
...
$vite = $null
try {
  Say '起 oph serve (127.0.0.1:7717)…'
  ...
  for ($i = 0; $i -lt 60 -and -not $token; $i++) {
    if ($serve.HasExited) { throw "oph serve 启动即退出（exit $($serve.ExitCode)），看上面的报错。" }
    if (Test-Path $serveLog) {
      $line = Select-String -Path $serveLog -Pattern '^OPH_AUTORESEARCH_TOKEN=(.+)$' -ErrorAction SilentlyContinue |
              Select-Object -First 1
      if ($line) { $token = $line.Matches[0].Groups[1].Value.Trim() }
    }
    if (-not $token) { Start-Sleep -Milliseconds 500 }
  }
  if (-not $token) { throw '30 秒内没等到 oph serve 打出令牌，启动失败。' }
  ...
  Write-Host ''
  Write-Host '  oph-autoresearch 已就绪' -ForegroundColor Green
  Write-Host "  $url"
  Write-Host '  局域网/手机接入看上面 oph serve 打出的地址与二维码（那条链路直连 7717，不过 vite）'
